    ...
    for row in range(tree_grid.shape[0]):
        for column in range(tree_grid.shape[1]):
            ...
            if row == 0:
                ...
                visibility_top = scenic_score_direction(tree, tree_grid[:row, column][::-1])
            if row == (tree_grid.shape[1] - 1):
                visibility_down = 0
            else:
                visibility_down = scenic_score_direction(tree, tree_grid[row + 1:, column])
            if column == 0:
                visibility_left = 0
            else:
                visibility_left = scenic_score_direction(tree, tree_grid[row, :column][::-1])
            if column == (tree_grid.shape[0] - 1):
                visibility_right = 0
            else:
                visibility_right = scenic_score_direction(tree, tree_grid[row, column + 1:])
            scenic_grid[row, column] = visibility_top * visibility_down * visibility_left * visibility_right
